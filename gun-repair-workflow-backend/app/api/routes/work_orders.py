...
def list_work_orders(db: DbSession) -> list[WorkOrderRead]:
...
def create_work_order(payload: WorkOrderCreate, db: DbSession) -> WorkOrderRead:
    service = WorkOrderService(db)
    try:
        work_order = service.create_work_order(gun_id=payload.gun_id)
...
def get_work_order(work_order_id: int, db: DbSession) -> WorkOrderRead:
    service = WorkOrderService(db)
    try:
...
def assign_worker(work_order_id: int, payload: AssignWorkerRequest, db: DbSession) -> WorkOrderRead:
    service = WorkOrderService(db)
    try:
        work_order = service.assign_worker(work_order_id=work_order_id, user_id=payload.user_id)
        return WorkOrderRead.model_validate(work_order)
    except (EntityNotFoundError, AuthorizationError, ConflictError) as exc:
        _raise_http_error(exc)
...
def request_component(work_order_id: int, payload: ComponentRequestCreate, db: DbSession) -> WorkOrderRead:
    service = WorkOrderService(db)
    try:
